...
const obtenerNoticias = (req, res) => {//funcion controladora se usa cuando un ususario entre en la ruta /api/galeria y recibe dos parametros req->infde la peticion(datos usu)rea-> obj con el que respodemos el navegador

    const filePath = path.join(__dirname, '../data', 'noticias.json');//Crea la ruta completa del archivo JSON llamado galeria.json, que está dentro de la carpeta data, y asi puede ser llamada en el frontend
    ...
    try {
        ...
        console.error('Error en obtenerNoticias.', error)// Esto se mostrará en la terminal del backend

        if(error.code === 'ENOENT') {
            return res.status(404).json({message: 'Error: Archivo noticia.json no encontrado en la ruta especificada.'})
        }
        if(error instanceof SyntaxError) {
            return res.status(500).json({message: 'Error: El archivo noticia.json tiene un formato JSON inválido.'})
        }
        res.status(500).json({message: 'Error interno del servidor al cargar la noticia.'})
    }
}
